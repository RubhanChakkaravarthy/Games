...
function EntityWalkState:update(dt)
    ...
    for k, object in pairs(objects) do
        if self.entity:collides(object) and object.solid then
            ...
            if self.entity.direction == 'left' then
                self.entity.x = math.max(self.entity.x + self.entity.walkSpeed * dt, object.x + object.width)
            elseif self.entity.direction == 'right' then
                self.entity.x = math.min(self.entity.x - self.entity.walkSpeed * dt, object.x - self.entity.width)
            elseif self.entity.direction == 'up' then
                -- player y position for collision is considered half from the bottom
                -- so resetting player position will cause jumping of the player
                self.entity.y = self.entity.y + self.entity.walkSpeed * dt
            elseif self.entity.direction == 'down' then
                self.entity.y = math.min(self.entity.y - self.entity.walkSpeed * dt, object.y - self.entity.height - 1)
            end
        end    
    end
