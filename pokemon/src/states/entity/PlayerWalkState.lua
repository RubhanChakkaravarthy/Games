...
function PlayerWalkState:checkForEncounter()
    ...
    if self.level.grassLayer.tiles[y][x].id == TILE_IDS['tall-grass'] then
        ...
        -- only increaseCounter if player moves above tall grass
        self.entity.stepCounter = self.entity.stepCounter + 1
        
        -- every step player moves increases the enemy encounter chance nearly squared
        if math.random(50 - math.min(50, self.entity.stepCounter * self.entity.stepCounter * 0.67)) == 1 then

            -- if encountered enemy then reset counter
            self.entity.stepCounter = 0
