...
function BattleState:init(player)
    ...
    local playerPokemon = self.player.party.pokemon[1]
    self.opponent = Opponent {
        party = Party {
            pokemon = {
                Pokemon(Pokemon.getRandomDef(), math.random(playerPokemon.level - 2, playerPokemon.level + 1))
            }
        }
    }

    self.playerSprite = BattleSprite(playerPokemon.battleSpriteBack, 
