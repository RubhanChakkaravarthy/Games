...
function Level:update(dt)
    ...
    if self.launchMarker.launched then
    	...
    	-- assume all aliens stopped moving
    	-- if any one alien is still moving, change it to false
    	local allAliensStoppedMoving = true
		...
		for k, alien in pairs(self.launchMarker.aliens) do
		    ...
		    -- if we not fired our player alien to the left or not greater than certain distance on the right or
		    -- it's not almost done rolling, player alien is still moving
		    if not (xPos < 0 or xPos > 2 * VIRTUAL_WIDTH or (math.abs(xVel) + math.abs(yVel) < 3)) then
		        allAliensStoppedMoving = false
		    end

		end

		-- if all player aliens stopped moving
		if allAliensStoppedMoving then
			for k, alien in pairs(self.launchMarker.aliens) do
				alien.body:destroy()
			end
