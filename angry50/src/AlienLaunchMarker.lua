...
function AlienLaunchMarker:init(world)
    ...
    -- whether power to make 3 aliens is used
...
function AlienLaunchMarker:update(dt)
    ...
    if not self.launched then
    	...
    	if love.keyboard.wasPressed('space') and not self.powerUsed then
    		local mainAlien = self.aliens[1]

    		-- top alien
    		table.insert(self.aliens, Alien(self.world, 'round', mainAlien.body:getX(),
    					mainAlien.body:getY() - 17.5, 'Player'))

    		self.aliens[2].body:setLinearVelocity(mainAlien.body:getLinearVelocity())
    		-- linear damping to make the alien slow down faster than original alien
    		-- slows down faster than bottom alien
    		self.aliens[2].body:setLinearDamping(0.8)
    		self.aliens[2].fixture:setRestitution(0.4)
    		self.aliens[2].body:setAngularDamping(1)

    		-- bottom alien
    		table.insert(self.aliens, Alien(self.world, 'round', mainAlien.body:getX(),
    					mainAlien.body:getY() + 17.5, 'Player'))

    		self.aliens[3].body:setLinearVelocity(mainAlien.body:getLinearVelocity())
    		-- linear damping to make the alien slow down faster than original alien
    		self.aliens[3].body:setLinearDamping(0.4)
    		self.aliens[3].fixture:setRestitution(0.4)
    		self.aliens[3].body:setAngularDamping(1)
